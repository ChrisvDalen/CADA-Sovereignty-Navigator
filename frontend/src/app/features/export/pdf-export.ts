...
import { CadaLevel, Meta, ReportResponse, ReportRow } from '../../core/models';
...
class PdfWriter {
  ...
  paragraph(
    text: string,
    options?: { size?: number; color?: [number, number, number]; style?: 'normal' | 'bold' | 'italic' },
  ) {
...
export function exportPdf(report: ReportResponse, meta: Meta) {
  ...
  const levelInfo = (level: CadaLevel) => meta.levelInfo[String(level)];
  ...
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(11);
  doc.text(`Organisatie: ${orgName}`, MARGIN, 160);
  doc.text(`Datum: ${today}`, MARGIN, 168);
  doc.text(`Aantal geanalyseerde toepassingen: ${rows.length}`, MARGIN, 176);
  ...
  writer.paragraph(
    `Voor ${orgName} zijn ${rows.length} cloudtoepassing(en) geprofileerd op datatype, toepasselijke regelgeving, impact en leveranciers. De tabel hieronder toont per toepassing het aanbevolen CADA-niveau, het met de huidige leverancier(s) haalbare niveau en de compliance-status.`,
  );
  writer.space(2);
  ...
  autoTable(doc, {
    ...
    head: [['Toepassing', 'Aanbevolen', 'Leverancier(s)', 'Haalbaar', 'Status']],
    body: rows.map((row) => [
      row.app.name,
    ...
    didParseCell: (data) => {
      if (data.section === 'body' && data.column.index === 4) {
        ...
        data.cell.styles.textColor =
          value === 'OK' ? [26, 127, 55] : value === 'GAP' ? [179, 38, 30] : [154, 103, 0];
      }
    },
  });
  ...
  for (let i = 2; i <= pageCount; i++) {
    ...
    doc.text(`CADA Sovereignty Navigator · ${orgName} · ${today}`, MARGIN, PAGE_H - 10);
    doc.text(`Pagina ${i} van ${pageCount}`, PAGE_W - MARGIN, PAGE_H - 10, { align: 'right' });
  }

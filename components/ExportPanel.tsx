...
export function ExportPanel({ assessment }: { assessment: AssessmentDto }) {
  ...
  return (
    <div className="grid gap-5 md:grid-cols-2">
      <div className="flex flex-col rounded-lg border border-line bg-wit p-6">
        <p className="font-mono text-xs font-medium text-kobalt">
          .PDF
        </p>
        ...
        <button
          ...
          className="mt-5 w-full border rounded border-kobalt bg-kobalt px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-kobalt-diep disabled:cursor-not-allowed disabled:border-line disabled:bg-porselein disabled:text-ink-faint"
        >
      ...
      <div className="flex flex-col rounded-lg border border-line bg-wit p-6">
        ...
        <button
          ...
          onClick={handleExcel}
          disabled={disabled || busy !== null}
          className="mt-5 w-full rounded border border-kobalt bg-wit px-4 py-2.5 text-sm font-semibold text-kobalt transition-colors hover:bg-kobalt-50 disabled:cursor-not-allowed disabled:border-line disabled:text-ink-faint"
        >
      ...
      {error && (
        <p className="rounded border border-alert/30 bg-alert-bg px-3 py-2 text-sm text-alert md:col-span-2">
          {error}
        </p>
      )}
      {disabled && (
